...
// regardless of rotation.
		{ { -0.7f, -0.7f }, { 0x00, 0xFF, 0x00 } }, { { 0.7f, -0.7f }, { 0x00,
				0x00, 0xFF } }, { { -0.7f, 0.7f }, { 0xFF, 0x00, 0x00 } }, { {
...
Renderer::Renderer() :
		mNumInstances(0), mLastFrameNs(0) {
...
void Renderer::calcSceneParams(unsigned int w, unsigned int h, float* offsets) {
	...
	const int ncells[2] =
			{ NCELLS_MAJOR, (int) floorf(NCELLS_MAJOR * aspect[1]) };
...
extern "C" {
JNIEXPORT void JNICALL Java_org_creepbros_game_minimirobot_GLES3JNILib_init(
		JNIEnv* env, jobject obj);
JNIEXPORT void JNICALL Java_org_creepbros_game_minimirobot_GLES3JNILib_resize(
		JNIEnv* env, jobject obj, jint width, jint height);
JNIEXPORT void JNICALL Java_org_creepbros_game_minimirobot_GLES3JNILib_step(
		JNIEnv* env, jobject obj);
}
;
...
Java_org_creepbros_game_minimirobot_GLES3JNILib_init(JNIEnv* env, jobject obj) {
	ALOGV("GLES3JNILib_init start");
	...
	ALOGV("GLES3JNILib_init end");
}

JNIEXPORT void JNICALL
Java_org_creepbros_game_minimirobot_GLES3JNILib_resize(JNIEnv* env, jobject obj,
		jint width, jint height) {
	ALOGV("GLES3JNILib_resize start");
	if (g_renderer) {
		g_renderer->resize(width, height);
	}
	ALOGV("GLES3JNILib_resize end");
}

JNIEXPORT void JNICALL
Java_org_creepbros_game_minimirobot_GLES3JNILib_step(JNIEnv* env, jobject obj) {
	if (g_renderer) {
		g_renderer->render();
	}
}
